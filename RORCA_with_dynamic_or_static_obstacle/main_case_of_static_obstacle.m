...
while true
    ...
    if V_current == zeros(size(V_current))
        break;              % Judge whether the target is reached
    end
    ...
    for i=1:size(obs_positions,1)
        ...
        eval(['obs_pos_',num2str(i) ,'(',num2str(t),',:) = obs_positions(',num2str(i),',:);']);   % Define and store the location coordinates of each obstacle
    end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for i=1:size(obs_positions,1)
        p=plot(obs_positions(i,1),obs_positions(i,2),'h','color',color(i,:));           % Display current obstacles
        axis([-10,70,-10,70]);
    ...
    for i=1:size(robot_positions,1)
        p=plot(robot_positions(i,1),robot_positions(i,2),'h','color',color(i,:));           % Display the current robots
        axis([-10,70,-10,70]);
